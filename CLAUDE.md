...
The Next.js app's "project root" is `src/` — `package.json`, `node_modules/`, all configs, and all source live there. The repo root holds only `infra/` (Dockerfile), `docker-compose.yml`, `.env`, and `src/`. **Inline paths below are relative to `src/`** (the app root) unless prefixed with `src/`; clickable links resolve from the repo root.
...
When changing the nav menu, edit [`partials/header.html`](src/static_site/archcraft/partials/header.html) and also apply the same change in [`home/index.html`](src/static_site/archcraft/home/index.html) (which carries its own embedded menu copies — the partial doesn't touch the home page header).
...
- [`public/chatbot.js`](src/public/chatbot.js) — vanilla JS widget, scripted Q&A, classifier
...
- [`scripts/build-partials.mjs`](src/scripts/build-partials.mjs) — injects `<script src="/chatbot.js" defer>` into every HTML page (idempotent, wrapped in `<!-- inject:chatbot -->...<!-- /inject:chatbot -->` markers)
