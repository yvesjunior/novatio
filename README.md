...
>
> **Keeping existing data:** if you already have submissions in the old containerized Postgres,
> copy them into the host DB **once, before first boot**, with [`migrate-db.sh`](migrate-db.sh):
> `TARGET_DATABASE_URL=postgres://user:pass@localhost:5432/nanopods ./migrate-db.sh` (run on the box
> with the old `db` container). It dumps the container DB — including the migration state — and loads
> it into the empty host DB, so the boot migration then no-ops and the rows are preserved.
...
| [`migrate-db.sh`](migrate-db.sh) | box with old `db` container | One-time copy of submissions from the old containerized Postgres → the host Postgres. Run before first boot against the host DB. |
